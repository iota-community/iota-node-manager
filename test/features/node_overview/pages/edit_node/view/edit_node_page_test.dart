...
void main() {
  ...
  setUpAll(() {
    registerFallbackValue(FakeEditNodeCubitState());
  ...
  group('EditNodePage', () {
    ...
    group('Edit / Create node flow', () {
      ...
      testWidgets('Tests the whole edit node flow if an node is edited',
        ...
        when(() => _nodeCubit.nodeRemoved(any())).thenAnswer(
          (_) async => <void>{},
        );
        ...
        final url = node.url;
        final urlInput =
            find.byKey(const Key('editNodeForm_urlInput_textField'));
        ...
        final jwtInput =
            find.byKey(const Key('editNodeForm_JwtInput_textField'));
